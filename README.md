...
<p align="center">
  ...
  <a href="https://github.com/ujix/noop/wiki/FAQ">❓&nbsp;FAQ</a> ·
...
|---|---|---|
| **macOS** | `NOOP.app` (see [Releases](../../releases)) or `brew install --cask ujix/noop/noop` | Apple Silicon + Intel. Drag to Applications. Not notarized — see **First launch on macOS** below. Homebrew needs a one-time `brew trust ujix/noop` first (Homebrew 6.0+) — see [Homebrew docs](docs/HOMEBREW.md). |
...
>   **Play Store → your profile icon → Play Protect → âڑ™ Settings**, toggle **"Scan apps with Play
...
| Strap | Status |
|---|---|
| **WHOOP 4.0** | âœ… The tested, supported path. Live HR, recovery, strain, sleep, history offload — the full experience. (v1.95 also unlocked sleep + recovery on the newer "v25" 4.0 firmware layout that earlier versions could only read live HR from.) |
| **WHOOP 5.0 / MG** | ًں§ھ **Live heart rate works** (confirmed on real hardware). Pick "WHOOP 5.0 / MG" before connecting — and see the pairing note below, because you can't just scan for it. Deeper 5/MG metrics (recovery, strain, sleep) are still being mapped; there's an opt-in **Settings → Experimental** toggle for 5/MG owners who want to help document the protocol. |
...
# Select the "Strand" scheme → Run (âŒکR). The built app is named NOOP.
```
...
**Bug reports:** please use **[GitHub Issues](https://github.com/ujix/noop/issues)** — there's a template, and they're tracked, deduped and linked to fixes (include a strap log).
...
If NOOP's useful to you, a â­گ genuinely helps it reach more WHOOP users — and it's the single best free way to support the project.
