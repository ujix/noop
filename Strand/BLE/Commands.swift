...
public enum WhoopCommand: UInt8, CaseIterable {
    ...
    /// The leading 0x01 is the form byte; bytes 5-6 are subseconds (always 0, minute-precision);
    /// bytes 7-8 are the haptic-mode field — confirmed required by 4.0 firmware via HCI wire
    /// capture of the official WHOOP app. Without them the strap ACKs but never buzzes.
    /// Always send SET_CLOCK (cmd 10) first so the strap RTC is UTC-correct.
    public static func setAlarmPayload(epochSec: UInt32) -> [UInt8] {
        [0x01,
         ...
         0x00, 0x00] // haptic-mode field (confirmed wire capture)
    }
